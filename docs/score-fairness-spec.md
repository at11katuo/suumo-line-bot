...
- [x] 是政実例: +38%割高の物件のスコアが大幅減点され、割安度がスコアに反映される
- [x] 現行36件の新旧スコア対照表をしゅんが承認済み
- [x] sashine docstringの設計前提が実装と一致
- [x] デプロイ初日の偽score_gainアラート対策が入っている
- [x] 全既存テスト＋新規テストがパス（期待値は手計算で先に確定）
- [x] 本番データdry_run（実/注入タグ明記）の目視承認

STEP1〜4完了・全DoD承認済み（2026-07-17〜18）。デプロイ手順も完了。

## 7. 【2026-07-18 追記】診断スクリプトの新旧比較はデプロイ完了後に無効

STEP1で作った `scripts/inspect_score_impact.py`（`check_score_impact.yml`）は、
「DBのresale_score＝旧式（割安度を含まない）」という**デプロイ前提**で、
DB値に段階加減点をもう一度適用して「新式」列を作る対照表ツールだった。

**デプロイが完了すると、この前提が崩れる。** 本番の`_resale_score`自体が
段階加減点を計算するようになるため、DBの`resale_score`列はすでに新式の
値になっている。そこにスクリプトが同じ加減点をもう一度適用すると、
**二重適用**になる。

実際に発生した事故: 是政4LDK（nc_20893454、実勢比+38.0%）が
`61 →(デプロイ)→ 41 →(スクリプトの二重適用)→ 21` と、-20点を2回
適用された値になって報告された（2026-07-18に発見・修正）。

### 教訓・恒久ルール

- **スコア公式（段階加減点の閾値・点数）は `reinfolib_resale._resale_score`
  の一箇所にのみ存在する。** 診断・調査スクリプトに公式を複製しては
  ならない。複製すると、本体側の変更（デプロイ）に追従できず、この
  事故のように「過去には正しかったロジックが、ある時点から黙って
  間違った値を出し続ける」形で壊れる。
- 対照表（新旧比較）のような「本体の変更前後を比較する」タイプの診断
  スクリプトは、**変更が本番にデプロイされた時点で役目を終える**。
  作りっぱなしで放置せず、デプロイ完了後は速やかに用途を見直すこと。
- `scripts/inspect_score_impact.py`は現在、DB値を加工せず表示する
  「週次観測用の定点観測ツール」（前回評価日比の差分・分布ヒストグラム）
  に役割を変更済み。ファイル名・ワークフロー名は変更churn回避のため
  維持しているが、中身は別物と理解すること。
